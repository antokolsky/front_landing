...
const STLViewer = async (elem, model) => {
  ...
  scene.add(
    new THREE.HemisphereLight(
      cfg.sphereLightColor,
      cfg.sphereLightColor,
      cfg.sphereLightPower
    )
  );

  const addLight = (param) => {
    const { x, y, z } = param;
    const object3d = new THREE.DirectionalLight(
      cfg.spotLightColor,
      cfg.spotLightPower
    );
...
export const STLViewerEnable = () => {
  const rootPath = process.env.NODE_ENV === "development" ? "./" : "./";
  const models = document.getElementsByClassName("stlviewer");
  for (let m of models) {
